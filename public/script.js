...
async function renderCalendar() {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        // สร้าง DateKey ในรูปแบบYYYY-MM-DD
    ...
    if (noteToDeleteId) { // ถ้ามี ID โน้ตที่จะลบ
        try {
            const response = await fetch(`${API_BASE_URL}/notes/${noteToDeleteId}`, { // เรียก API DELETE เพื่อลบโน้ต
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }, // เพิ่ม Header นี้
                body: JSON.stringify({ user_name: loggedInUser.name }) // ส่ง user_name เพื่อตรวจสอบสิทธิ์
            });
            if (response.ok) {
